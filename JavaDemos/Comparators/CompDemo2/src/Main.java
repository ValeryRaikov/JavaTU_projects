...
public class Main {
    public static void main(String[] args) {
        ...
        group.sort(new SortByName());
        System.out.println("Sort by name");
        for (Student st : group) {
            System.out.println(st);
        }
        group.sort(new SortByNumber());
        ...
        List<Student> list = Arrays.asList(
                new Student(101, "Ivan Ivanov", 5.55f),
                new Student(202, "Lili Ivanova", 5.55f),
                new Student(303, "Georgi Petrov", 5.65f)
        );

        list.sort(new SortByMark().thenComparing(new SortByName()));
