...
public class Main {
    ...
    public static void addDevice(ElectronicDevice device) {
        ...
        if (devices != null && !devices.contains(device)) {
            devices.add(device);
            saveDevicesToFile(devices);
            System.out.format("Successfully added %s to file.%n", device.getDeviceType());
        } else {
            System.out.format("This %s is already in the file!%n", device.getDeviceType());
        }
    }
        ...
        if (devices != null) {
            for (ElectronicDevice currDevice : devices) {
                if (currDevice.equals(device)) {
                    System.out.format("Found such %s!%n", device.getDeviceType());
            ...
            System.out.format("No such %s found!%n", device.getDeviceType());
        } else {
    ...
    public static void printSortedDevices(List<ElectronicDevice> devices) {
        ...
        switch (choice) {
            ...
            case 3:
                sortedDevices = devices.stream().sorted(
                        Comparator.comparing(ElectronicDevice::getDeviceType)
                        .thenComparingInt(ElectronicDevice::getYear))
                        .toList();
                break;
            case 4:
                sortedDevices = devices.stream().sorted(
